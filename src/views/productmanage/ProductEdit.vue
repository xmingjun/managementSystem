...
export default {
  ...
  created() {
    ...
    /* 将获取的本地数据回填到表单 */
    let editRow = JSON.parse(window.sessionStorage.getItem("editRow"));
    // console.log(editRow);
    this.formData = {
      id:editRow.id,
      goodsname: editRow.goodsname, //商品名称
      goodscategory: editRow.goodscategory, //商品分类
      goodsfeature: JSON.parse(editRow.goodsfeature), //商品特色
      isPromotion: editRow.isPromotion, //是否促销
      goodsstandard:
        JSON.parse(editRow.standard)[0].goodsstandard === "单规格"
          ? "单规格"
          : "多规格", //食品规格
      packingexpense:
        JSON.parse(editRow.standard)[0].goodsstandard === "单规格"
          ? JSON.parse(editRow.standard)[0].packingexpense
          : "", //单规格包装费
      goodsPrice:
        JSON.parse(editRow.standard)[0].goodsstandard === "单规格"
          ? JSON.parse(editRow.standard)[0].goodsPrice
          : "", //单规格价格
      goodsdesc: editRow.goodsdesc, //商品描述
      imgUrl: editRow.imgUrl, //商品图片地址
      //多规格
      tableData:
        JSON.parse(editRow.standard)[0].goodsstandard !== "单规格"
          ? JSON.parse(editRow.standard)
          : [
              {
                goodsstandard: "默认", //规格
                packingexpense: "0", //包装费
